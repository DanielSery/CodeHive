...
function createCommitPushPty(mainWindow, { wtPath, message, description }) {
  const cwd = path.resolve(wtPath);
  const isWin = process.platform === 'win32';
  const os = require('os');
  const scriptPath = path.join(os.tmpdir(), `codehive-commit-${Date.now()}${isWin ? '.cmd' : '.sh'}`);

  // Build commit message with optional description
  let commitMsg = message;
  if (description) commitMsg += '\n\n' + description;

  const lines = [];
  if (isWin) {
    lines.push('@echo off');
    lines.push('echo Adding all changes...');
    lines.push('git add .');
    lines.push('echo.');
    lines.push('echo Status:');
    lines.push('git status --short');
    lines.push('echo.');
    lines.push('git diff --cached --stat');
    lines.push('echo.');
    lines.push('echo Committing...');
    // Use a temp file for the commit message to handle multiline and special chars
    const msgFile = path.join(os.tmpdir(), `codehive-msg-${Date.now()}.txt`);
    fs.writeFileSync(msgFile, commitMsg, { encoding: 'utf8' });
    lines.push(`git commit -F "${msgFile}"`);
    lines.push('if errorlevel 1 (');
    lines.push(`  del "${msgFile}" 2>nul`);
    lines.push('  echo.');
    lines.push('  echo No changes to commit.');
    lines.push('  exit /b 1');
    lines.push(')');
    lines.push(`del "${msgFile}" 2>nul`);
    lines.push('echo.');
    lines.push('echo Pushing...');
    lines.push('git push 2>nul');
    lines.push('if errorlevel 1 (');
    lines.push('  echo Setting upstream and pushing...');
    lines.push('  for /f "tokens=*" %%b in (\'git rev-parse --abbrev-ref HEAD\') do git push -u origin %%b');
    lines.push(')');
    lines.push('echo.');
    ...
    const msgFile = path.join(os.tmpdir(), `codehive-msg-${Date.now()}.txt`);
    fs.writeFileSync(msgFile, commitMsg, { encoding: 'utf8' });
    lines.push(`git commit -F "${msgFile}" || { rm -f "${msgFile}"; echo "No changes to commit."; exit 1; }`);
    lines.push(`rm -f "${msgFile}"`);
    lines.push('echo ""');
    lines.push('echo "Pushing..."');
    lines.push('BRANCH=$(git rev-parse --abbrev-ref HEAD)');
    lines.push('git push 2>/dev/null || git push -u origin "$BRANCH"');
    lines.push('echo ""');
    lines.push('echo "=== COMMIT AND PUSH COMPLETE ==="');
  }
  ...
  proc.onExit(({ exitCode }) => {
    try { fs.unlinkSync(scriptPath); } catch {}
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('commitPush:exit', { exitCode, wtPath });
    }
  });
...
function createPullRequestPty(mainWindow, { wtPath, targetBranch, title, description }) {
  const cwd = path.resolve(wtPath);
  const isWin = process.platform === 'win32';
  const os = require('os');
  const scriptPath = path.join(os.tmpdir(), `codehive-pr-${Date.now()}${isWin ? '.cmd' : '.sh'}`);
  ...
  if (isWin) {
    lines.push('@echo off');
    lines.push('echo Pushing latest changes...');
    lines.push('git push 2>nul');
    lines.push('if errorlevel 1 (');
    lines.push('  for /f "tokens=*" %%b in (\'git rev-parse --abbrev-ref HEAD\') do git push -u origin %%b');
    lines.push(')');
    lines.push('echo.');
    lines.push('echo Creating pull request...');
    lines.push(`az repos pr create --target-branch "${targetBranch}" --title "${escapedTitle}"${descArg} --auto-complete --open`);
    lines.push('if errorlevel 1 (');
    lines.push('  echo.');
    lines.push('  echo === PR CREATION FAILED ===');
    lines.push('  exit /b 1');
    lines.push(')');
    lines.push('echo.');
    ...
    lines.push('echo ""');
    lines.push('echo "Creating pull request..."');
    lines.push(`az repos pr create --target-branch "${targetBranch}" --title "${escapedTitle}"${descArg} --auto-complete --open || { echo ""; echo "=== PR CREATION FAILED ==="; exit 1; }`);
    lines.push('echo ""');
    lines.push('echo "=== PULL REQUEST CREATED ==="');
  }
  ...
  proc.onData((data) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('pullRequest:data', data);
    }
  });
  ...
  proc.onExit(({ exitCode }) => {
    try { fs.unlinkSync(scriptPath); } catch {}
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('pullRequest:exit', { exitCode, wtPath });
    }
  });
...
module.exports = { createWorktreePty, createClonePty, createDeletePty, createWorktreeRemovePty, createWorktreeSwitchPty, createCommitPushPty, createPullRequestPty };
