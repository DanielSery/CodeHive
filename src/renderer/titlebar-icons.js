import {
  DOT_SYNC_SVG, DOT_CREATE_PR_SVG, DOT_OPEN_PR_SVG, DOT_COMPLETE_PR_SVG,
  DOT_RESOLVE_TASK_SVG, DOT_PIPELINE_SVG, DOT_OPEN_TASK_SVG, DOT_SWITCH_SVG,
  DOT_SET_TASK_SVG, DOT_DISCONNECT_SVG, DOT_OPEN_MERGED_PR_SVG, BIN_ICON_SVG,
...
const BUTTON_ICONS = {
  ...
  'btn-check-updates': UPDATE_SVG, // may be replaced below for dev builds
  'btn-titlebar-commit': DOT_SYNC_SVG,
  ...
  'btn-titlebar-disconnect': DOT_DISCONNECT_SVG,
...
updaterAPI.isPackaged().then(isPackaged => {
  if (!isPackaged) {
    ...
    if (updateButton) { updateButton.innerHTML = PUBLISH_SVG; }
  }
});
